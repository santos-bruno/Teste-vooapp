...
/**
 * Renderiza o GINGA: menu, seleção, contagem, dança (pictograma neon estilo
 * Just Dance), resultado e ranking. Palco colorido, holofotes e efeitos.
...
class DanceRenderer {
    ...
    fun render(canvas: Canvas, e: DanceEngine) {
        ...
        val floorY = h * 0.68f
        val pulse = if (e.screen == Screen.PLAYING) e.beatPulse else (0.5f + 0.5f * sin(time * 2.2f))

        drawStage(canvas, w, h, floorY, time, pulse)
    ...
    private fun drawStage(canvas: Canvas, w: Int, h: Int, floorY: Float, time: Float, pulse: Float) {
    ...
    private fun drawScene(canvas: Canvas, w: Int, h: Int, u: Float, e: DanceEngine, dim: Boolean) {
        ...
        pictogram(canvas, cx, cy, s, e.currentMove, e.song.hue, u, true)
    ...
    private fun arrow(c: Canvas, x: Float, y: Float, r: Float, a0: Float, a1: Float) {
        ...
        c.drawArc(RectF(x - r, y - r, x + r, y + r), Math.toDegrees(a0.toDouble()).toFloat(), Math.toDegrees((a1 - a0).toDouble()).toFloat(), false, stroke)
    ...
    private fun logo(canvas: Canvas, w: Int, h: Int, u: Float, time: Float) {
        ...
        pictogram(canvas, cx, h * 0.30f, h * 0.055f, Move.ALL[3], 315, u, true)
    }
